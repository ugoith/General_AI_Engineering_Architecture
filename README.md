...
| **改了 A 忘了 B**（"必须同步更新 C"只是句建议） | 影响矩阵带**机械判据**（`when`）：改了模型/接口/依赖/构建配置会被自动识别；收尾时 `review --task` 核对 `mustUpdate` 里的文件是否真的改了 | 漏同步报 `task-impact-not-updated`——这是变更协议里此前唯一没有验收的一步 |
