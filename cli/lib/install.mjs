...
export function buildQuickstartPrompt(opts) {
  ...
  lines.push('## 你要做的事');
  lines.push('');
  lines.push('### 第 1 步：接入（一条命令，不要手工创建目录）');
  lines.push('');
  lines.push('**最简：不需要克隆框架**（npx 会临时取一份并直接运行）：');
  lines.push('');
  lines.push('```bash');
  lines.push('npx -y github:ugoith/General_AI_Engineering_Architecture install --root .');
  lines.push('```');
  lines.push('');
  lines.push('如果 npx 不可用（离线环境、或公司网络限制），再改用克隆方式：');
  lines.push('');
  lines.push('```bash');
  ...
  lines.push('```');
  lines.push('');
  lines.push('本机**已有一份框架**时不要重复克隆，把上面的命令换成使用该目录即可（路径由用户提供）。');
  lines.push('');
  ...
  lines.push('');
  lines.push('```bash');
  lines.push(`npx -y github:ugoith/General_AI_Engineering_Architecture install --root . --pack ${detection.packId ?? '<pack-id>'} --name ${name}${engine ? ` --${engine.key} "${engine.value}"` : ''}`);
  lines.push('```');
  lines.push('');
