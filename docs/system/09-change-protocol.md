...
3. **收尾**：`ai-arch review --task`（对账本次任务：前提是否失效、摘要/注册表补了没有、证据填了没有）
   → `ai-arch index --stale`（按上一步清单补摘要）→ `ai-arch review --drift`（终检无残留漂移）。
4. **发现遗漏时**：往影响矩阵**加一条规则**，而不是提醒大家注意。规则比记性可靠。

> 第 3 步的顺序是刻意的：收尾对账必须在回写索引**之前**跑，否则它看到的已经是修复后的状态。
